...
BOT_NAME = "FullHouseRangeTracker"
...
# ---------------------------------------------------------------------------
# Bayesian range tracking — see Southey et al. UAI 2005 "Bayes' Bluff"
# Per opponent: maintain weight vector over 169 canonical preflop hand classes.
# On each action, multiply by P(action | hand_class). Use as sampling
# distribution in equity Monte Carlo.
# ---------------------------------------------------------------------------

def _build_hand_class_tables():
    """Build the 169 canonical preflop hand classes and their combos.

    Returns (classes, class_score, class_combos):
    - classes: list[str] of 169 keys like 'AA', 'AKs', 'AKo'
    - class_score: dict[str, float] preflop strength score per class
    - class_combos: dict[str, list[tuple[str, str]]] specific card-string combos
    """
    classes = []
    class_score = {}
    class_combos = {}
    suits = "shdc"
    ranks_high_to_low = "AKQJT98765432"
    for i, hi in enumerate(ranks_high_to_low):
        for j, lo in enumerate(ranks_high_to_low):
            if i > j:
                continue  # we'll generate the same class on swap
            if i == j:
                key = hi + lo  # pair
                combos = []
                for a in range(4):
                    for b in range(a + 1, 4):
                        combos.append((hi + suits[a], lo + suits[b]))
            else:
                suited_key = hi + lo + "s"
                classes.append(suited_key)
                class_combos[suited_key] = [(hi + s, lo + s) for s in suits]
                offsuit_key = hi + lo + "o"
                off_combos = []
                for s1 in suits:
                    for s2 in suits:
                        if s1 != s2:
                            off_combos.append((hi + s1, lo + s2))
                class_combos[offsuit_key] = off_combos
                classes.append(offsuit_key)
                # score: use a representative combo
                class_score[suited_key] = _preflop_score(class_combos[suited_key][0])
                class_score[offsuit_key] = _preflop_score(class_combos[offsuit_key][0])
                continue
            classes.append(key)
            class_combos[key] = combos
            class_score[key] = _preflop_score((hi + "s", lo + "h"))
    return classes, class_score, class_combos


def _action_likelihood(score, action, raises_seen):
    """P(action | preflop_score, raises_already_made_this_street).

    Calibrated heuristic anchored at a generic open-raising prior. Stronger
    hands raise more, weaker hands fold more. When raises have already
    occurred, the relevant action shifts (raise becomes 3-bet, etc).
    """
    s = max(0.0, min(100.0, score))
    # Smooth raise-propensity curve: 0.02 at score 20, 0.5 at score 60, 0.95 at score 90
    raise_p = 0.02 + 0.93 / (1.0 + pow(2.71828, (60.0 - s) / 9.0))
    # Tighter when 3-bet/4-bet: need stronger hands to keep going
    if raises_seen >= 1:
        raise_p = raise_p ** (1.0 + 0.6 * raises_seen)
    call_p_raw = 0.04 + 0.45 * max(0.0, 1.0 - abs(s - 55.0) / 35.0)
    if raises_seen >= 1:
        call_p_raw *= 0.75
    fold_p_raw = max(0.01, 1.0 - raise_p - call_p_raw)
    total = raise_p + call_p_raw + fold_p_raw
    raise_p /= total; call_p_raw /= total; fold_p_raw /= total
    ...
    if action == "fold":
        return fold_p_raw
    if action == "check":
        # Check ≈ no information (small lean toward weaker)
        return 0.5 + 0.2 * (1.0 - raise_p)
    return 0.5


def _compute_range_weights(state, seat, opp_stats):
    """Compute P(hand_class | observed actions) for opponent at this seat.

    Returns a dict mapping hand_class -> normalized weight. Empty if no info.
    """
    weights = {cls: 1.0 for cls in _HAND_CLASSES}
    actions = state.get("action_log") or []

    # Walk the action log in order, tracking raises_seen per street
    street_raises = 0
    last_street = None
    for entry in actions:
        if entry.get("seat") != seat:
            # Track raises on the table to model 3-bet/4-bet pressure
        ...
        for cls in list(weights.keys()):
            score = _HAND_CLASS_SCORE.get(cls, 50.0)
            weights[cls] *= max(0.001, _action_likelihood(score, act, street_raises))
        if act in ("raise", "all_in"):
            street_raises += 1

    # Opponent-type adjustment: if observed loose, widen distribution toward lower scores
    bot_id = None
    for p in state.get("players") or []:
        if p.get("seat") == seat:
            bot_id = p.get("bot_id")
            break
    if bot_id and opp_stats.get(bot_id, {}).get("actions", 0) >= 12:
        s = opp_stats[bot_id]
        ra = s.get("raises", 0) / max(1, s["actions"])
        ca = s.get("calls", 0) / max(1, s["actions"])
        if ra > 0.55:
            # Maniac: flatten the distribution
            for cls in weights:
                weights[cls] = weights[cls] ** 0.55
        elif ra < 0.18 and ca < 0.30:
            # Nit: sharpen
            for cls in weights:
                weights[cls] = weights[cls] ** 1.5

    total = sum(weights.values())
    if total <= 0:
        return {cls: 1.0 / len(_HAND_CLASSES) for cls in _HAND_CLASSES}
    return {cls: w / total for cls, w in weights.items()}


def _sample_from_range(weights, used, rng):
    """Sample a (c1_str, c2_str) hand from the weighted hand-class distribution.

    Uses up to 8 retries per class draw if all combos for that class are
    blocked by already-used cards. Falls back to highest-weighted class on
    persistent failure.
    """
    ...
    for _ in range(8):
        ...
        cum = 0.0
        chosen_cls = items[-1][0]
        for cls, w in items:
            cum += w
            if cum >= target:
                chosen_cls = cls
                break
        combos = _HAND_CLASS_COMBOS.get(chosen_cls, [])
...
def _estimate_equity(state, opponents):
    ...
    weights_per_opp = [
        _compute_range_weights(state, seat, _OPPONENTS) for seat in seats[:opponents]
    ]
    ...
    for _ in range(iterations):
        used = set(str(c) for c in hero + board)
        opp_hands = []
        ok = True
        for idx, weights in enumerate(weights_per_opp):
            combo = _sample_from_range(weights, used, rng)
            if combo is None:
                # Fall back to floor-based legacy sampling for this opponent
                hand = _sample_range_hand(deck, used, floors[idx], rng)
                if hand is None:
                    ok = False
                    break
                ...
                continue
            c1, c2 = combo
            opp_hands.append((_card(c1), _card(c2)))
            used.add(c1)
            used.add(c2)
        if not ok:
            continue
...
_HAND_CLASSES, _HAND_CLASS_SCORE, _HAND_CLASS_COMBOS = _build_hand_class_tables()
