...
def _decide_preflop(state):
    ...
    if raises == 0:
        ...
        if table["actions"] >= 16 and table["fold_rate"] > 0.42:
            threshold -= 3
        if table["actions"] >= 16 and pressure > 0.52:
            threshold += 2
    ...
    if pressure > 0.54:
        threebet_bar -= 3
        continue_bar -= 2
    elif table["actions"] >= 16 and pressure < 0.24:
        threebet_bar += 4
        continue_bar += 3
    ...
    if (
        ...
        and stack_bb >= 24
        and opponents <= 2
        and _roll(state, "3bet_bluff") < (0.19 if pressure < 0.44 else 0.12)
    ):
...
def _decide_postflop(state):
    ...
    if not state.get("can_check") and risk_mode == "fold":
        return {"action": "fold"}
    # If can_check (no bet to face), never fold — but the call_only mode still suppresses raises.
    ...
    if state.get("can_check"):
        # When the PFR-committed gate is engaged, skip marginal value bets entirely
        # so we don't pile more chips in with made<4. Strong made hands still bet.
        gate_throttle = (risk_mode == "call_only")
        if made >= 6 or equity >= 0.82:
            return _value_bet(state, pot, stack, equity, made, wet, opponents, spr)
        if made >= 4 and equity >= 0.67:
            return _value_bet(state, pot, stack, equity, made, wet, opponents, spr)
        if not gate_throttle and made == 3 and equity >= 0.60 and opponents <= 2:
            return _value_bet(state, pot, stack, equity, made, wet, opponents, spr)
        if not gate_throttle and made == 2 and equity >= 0.56 and opponents <= 2 and _roll(state, "thin_two_pair") < 0.62:
            return _value_bet(state, pot, stack, equity, made, wet, opponents, spr)
        if not gate_throttle and made == 1 and equity >= 0.63 and opponents == 1 and _roll(state, "thin_pair") < 0.34:
        ...
        if solver_action:
            return _commitment_safe_bet(state, pot, stack, equity, made, solver_action)
        ...
        if street != "river" and draw >= 3 and equity >= 0.34 and opponents <= 2:
            if _roll(state, "semi_bluff") < 0.24 + fold_equity:
                action = _semi_bluff(state, pot, stack, wet, spr)
                return _commitment_safe_bet(state, pot, stack, equity, made, action)
    ...
    if street == "river":
        if made <= 1 and owed > pot * 0.30 and villain["raise_rate"] < 0.45:
            return {"action": "fold"}
        if made == 1 and price <= 0.13 and villain["raise_rate"] > 0.52:
            return {"action": "call"}
    ...
    if (made >= 4 and equity >= 0.76) or (made >= 3 and equity >= 0.82 and opponents <= 2):
        ...
        if _roll(state, "value_raise") < (0.48 if opponents <= 2 else 0.24):
            ...
            action = _commitment_safe_bet(state, pot, stack, equity, made, action)
            return _risk_limited_action(action, risk_mode)
        return {"action": "call"}
    ...
    if solver_action:
        solver_action = _commitment_safe_bet(state, pot, stack, equity, made, solver_action)
    ...
    if street != "river" and draw >= 3 and price <= 0.19 and owed <= stack * 0.18:
        return {"action": "call"}
    if made == 2 and price <= 0.18 and opponents <= 2:
        return {"action": "call"}
    return {"action": "fold"}
...
def _my_invested_this_hand(state):
    """Sum chips we have put in this hand (blinds + calls + raise totals - already counted prior streets).

    The action_log records absolute totals for raise/all_in (target-to) and increments for call.
    We approximate cumulative invested by summing call/blind amounts and tracking the running
    bet_this_street ceiling per street for raise/all_in.
    """
    mine = state.get("seat_to_act")
    log = state.get("action_log") or []
    invested = 0
    street_max = 0
    last_street_idx = -1
    streets_seen = {"preflop": 0}
    current_street = "preflop"
    # action_log doesn't include street markers; instead infer by ordering: blinds first, then preflop, etc.
    # Approximation: sum amounts of my call/blind entries directly, plus running max of my raise/all_in
    # within each contiguous segment of raises. Simpler robust estimate:
    my_bet_now = int(state.get("your_bet_this_street", 0))
    prior_street_committed = 0
    last_my_total = 0
    for entry in log:
        if entry.get("seat") != mine:
            continue
        action = entry.get("action")
        ...
        if action in ("small_blind", "big_blind", "call"):
            ...
            # raise/all_in amount is the target total bet for this street; we need the increment
            # from prior bet on this street. Approximate as max(amount - last_my_total, 0).
            invested += max(0, amount - last_my_total)
            last_my_total = amount
    return invested


def _is_preflop_aggressor(state):
    """True if our seat made the last preflop raise/all-in before the flop."""
    mine = state.get("seat_to_act")
    log = state.get("action_log") or []
    aggressor = None
    for entry in log:
        ...
        if action in ("small_blind", "big_blind"):
            continue
        # Stop scanning once we leave preflop. action_log is flat (no street markers),
        # but board cards being non-empty implies postflop; we want preflop aggressor only.
        if action in ("raise", "all_in"):
            aggressor = entry.get("seat")
        # Once we see any postflop-only action it's still ambiguous without street markers;
        # use a heuristic: take the LAST raise/all_in that occurred before community cards came down.
    # Since action_log lacks street markers, we approximate: any raise before the flop means
    # the LAST raiser pre-flop was the aggressor. We assume the first community card appears
    # only after preflop betting closes, so all raises while board was empty are preflop raises.
    # We can't directly know board state per entry, so use a simpler proxy: if community_cards
    # exists, scan only entries up to a point where every active player has matched the top bet.
    # Practical shortcut: track raise sequence; the LAST preflop raiser is the one whose
    # raise/all_in is followed by only calls/checks/folds before the first 'flop-like' gap.
    # Given the engine emits raises in order, the LAST raise in the log that came before any
    # community card was dealt is the PFR. We approximate via the heuristic: if community
    # cards exist, find the LAST raise/all_in in the log whose index is small enough — fall
    # back to assuming the LAST raise before all active players' bet_this_street are equal.
    # Simpler still: if street == "preflop", any raise we made is the PFR action; if postflop,
    # we rely on a per-hand snapshot kept in _PFR_TRACKER below.
    return aggressor == mine


_PFR_TRACKER = {"hand_id": None, "pfr_seat": None}


def _update_pfr_tracker(state):
    """Snapshot the preflop aggressor once we are postflop. Called from _decide_postflop."""
    hand_id = state.get("hand_id")
    if _PFR_TRACKER.get("hand_id") == hand_id and _PFR_TRACKER.get("pfr_seat") is not None:
        return _PFR_TRACKER["pfr_seat"]
    log = state.get("action_log") or []
    pfr = None
    if state.get("street") == "preflop":
        # On preflop, our latest raise (if any) would set us as PFR going forward.
        for entry in log:
            if entry.get("action") in ("raise", "all_in"):
                pfr = entry.get("seat")
        # Still preflop: don't lock in yet.
        return pfr
    # Postflop: action_log starts with blinds + preflop actions; we capture the last raise
    # that occurred before the first call that closed action (heuristic = last raise).
    for entry in log:
        if entry.get("action") in ("raise", "all_in"):
            pfr = entry.get("seat")
    _PFR_TRACKER["hand_id"] = hand_id
    _PFR_TRACKER["pfr_seat"] = pfr
    return pfr


def _we_are_pfr(state):
    """True iff our seat was the preflop aggressor for this hand."""
    pfr = _update_pfr_tracker(state)
    return pfr is not None and pfr == state.get("seat_to_act")


def _jam_commitment_safe(state, pot, stack, bet_amount, equity, made):
    """Forward-looking check: if villain shoves over our bet, can we profitably call?

    Returns True if our bet is safe (we could profitably call a jam, OR our hand is strong
    enough to commit regardless). Returns False if we'd be priced into a -EV jam-call.
    """
    if bet_amount <= 0:
        return True
    # Strong made hands commit freely.
    if made >= 4:
        return True
    # Assume villain shoves their remaining stack; conservative model uses our remaining stack
    # (since they likely have >= ours when they jam).
    my_bet = int(state.get("your_bet_this_street", 0))
    chips_we_put_in_for_bet = max(0, bet_amount - my_bet)
    pot_after_my_bet = pot + chips_we_put_in_for_bet
    remaining_stack_after_bet = max(0, stack - chips_we_put_in_for_bet)
    if remaining_stack_after_bet <= 0:
        # We are already all-in via this bet — separate logic handles that.
        return True
    # Worst-case jam size = our remaining stack (calling caps at our stack).
    call_amount = remaining_stack_after_bet
    pot_facing_call = pot_after_my_bet + call_amount  # villain matches our bet then jams
    needed_equity = call_amount / max(1, call_amount + pot_facing_call)
    safety_margin = 0.05
    return equity >= (needed_equity - safety_margin)


def _commitment_safe_bet(state, pot, stack, equity, made, action):
    """Wrap a value-bet/raise action. If the action represents a bet >=50% pot but villain-jam
    pot odds exceed our equity (with safety margin), downsize to a small bet or check.
    """
    if not isinstance(action, dict):
        return action
    name = action.get("action")
    if name not in ("raise", "all_in"):
        return action
    if made >= 4:
        return action
    if name == "all_in":
        # Already shoving — only protect if we are clearly behind needed equity for a snap-call.
        # Compute as: we are putting in our whole stack; safe iff equity >= ~ pot odds of our
        # commit divided by final pot. Skip the check when we already have shown commitment.
        my_bet = int(state.get("your_bet_this_street", 0))
        commit = stack
        final_pot = pot + commit  # villain may or may not call; use bet-sizing perspective
        needed = commit / max(1, commit + final_pot)
        if equity < needed - 0.05 and made < 4:
            # Downsize to a small bet instead of jamming a marginal hand.
            return _small_bet(state, pot, stack)
        return action
    # raise action with explicit amount
    amount = int(action.get("amount") or 0)
    my_bet = int(state.get("your_bet_this_street", 0))
    incremental = max(0, amount - my_bet)
    # Skip the check for small bets (<50% pot increment).
    if incremental < pot * 0.50:
        return action
    if _jam_commitment_safe(state, pot, stack, amount, equity, made):
        return action
    # Downsize: cap incremental at 30% pot; if even that isn't safe, check/call instead.
    capped_amount = my_bet + int(pot * 0.30)
    if capped_amount <= my_bet + int(state.get("amount_owed", 0)):
        # Can't downsize below required call — fall back to check or call.
        if state.get("can_check"):
            return {"action": "check"}
        return {"action": "call"}
    return _raise_to(state, capped_amount)
...
def _value_bet(state, pot, stack, equity, made, wet, opponents, spr):
    ...
    if spr <= 1.05 and (made >= 4 or equity >= 0.78):
        return {"action": "all_in"}
    raw = _raise_to(state, int(pot * min(1.08, frac)))
    return _commitment_safe_bet(state, pot, stack, equity, made, raw)
...
def _risk_gate_mode(state, made, draw, equity, opponents, stack, owed, price, spr, villain):
    if owed <= 0:
        # Even when not facing a bet, if we are the preflop aggressor and have already
        # committed >25% of our stack, lock down marginal value-raises.
        if _we_are_pfr(state) and made < 4:
            invested = _my_invested_this_hand(state)
            if invested >= 0.25 * (stack + invested):
                # Treat as call_only so the surrounding code downsizes raises to calls/checks.
                return "call_only"
        return "open"
    ...
    # When WE are the preflop aggressor and have already invested >25% of stack,
    # tighten the gate: lower the "skip the gate" threshold and require more equity.
    pfr_committed = False
    if _we_are_pfr(state):
        invested = _my_invested_this_hand(state)
        if invested >= 0.25 * (stack + invested):
            pfr_committed = True

    if not pfr_committed and risk_frac < large_risk and price <= 0.24:
        return "open"

    min_equity = _risk_min_equity(state, made, draw, equity, opponents, stack, risk_frac, villain)
    if pfr_committed:
        # Demand extra cushion when we built the pot ourselves.
        min_equity += 0.03
    ...
    if stack >= int(defaults.get("lead_stack", 13000)) and made < 4 and equity < min_equity + 0.050:
        return "call_only"
    if pfr_committed and made < 4 and equity < min_equity + 0.060:
        return "call_only"
    return "open"
...
def _draw_score(state):
    ...
    for suit, count in suits.items():
        has_hero_suit = any(card[1] == suit for card in hero)
        if count >= 4 and has_hero_suit:
            score += 2
        elif count == 3 and has_hero_suit and state.get("street") == "flop":
            score += 1
...
def _cfr_response_action(state, made, draw, equity, opponents, pot, stack, owed, price, villain):
    ...
    if state.get("street") != "river" and draw >= 3 and price <= float(response.get("draw_peel_price", 0.20)):
        return {"action": "call"}
    if made >= 1 and price <= float(response.get("pair_peel_price", 0.15)):
        return {"action": "call"}
    if price >= float(response.get("fold_price", 0.36)) and made <= 1 and draw <= 1:
        return {"action": "fold"}
    return None
