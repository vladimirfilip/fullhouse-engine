...
def _board_texture_name(board):
    # Expanded 15-archetype classifier (was 6). Splits paired by pair-rank,
    # monotone/two-tone by top card, splits dry_high into ace-high vs broadway,
    # and adds straight_connected for high-coordinated boards. Each archetype
    # in postflop_solver_digest.json has matched bucket data.
    flop = list(board[:3])
    if len(flop) < 3:
        return "rainbow_mid"
    ...
    max_rank = max(ranks)
    if len(set(ranks)) < len(ranks):
        pair_rank = next(r for r in ranks if ranks.count(r) >= 2)
        kicker = max(r for r in ranks if r != pair_rank) if len(set(ranks)) > 1 else pair_rank
        if pair_rank >= 12 or (pair_rank >= 7 and kicker >= 13):
            return "paired_high"
        if pair_rank >= 7:
            return "paired_mid"
        return "paired_low"
    ...
    if max(suits.values()) >= 3:
        return "monotone_high" if max_rank >= 11 else "monotone_low"
    ...
    if straightiness >= 3 and high_cards >= 2:
        return "wet_broadway"
    if straightiness >= 3:
        if max_rank >= 10 or high_cards >= 1:
            return "straight_connected"
        return "low_connected"

    if max_rank >= 14:
        return "two_tone_high" if two_tone else "dry_high_ace"
    if max_rank >= 13:
        return "two_tone_high" if two_tone else "dry_high_broadway"
    if two_tone:
        if max_rank >= 10:
            return "two_tone_high"
        return "two_tone_low"
    if max_rank <= 8:
        return "rainbow_low"
    return "rainbow_mid"
