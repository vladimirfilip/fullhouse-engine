...
BOT_NAME = "FullHouseOppProfileV6"
...
def _load_opponent_profiles():
    """Per-opponent overrides keyed by bot_id.

    Format:
        {
          "bot_id_1": {"min_equity_adj": -0.03, "value_bar_adj": +0.02, ...},
          "bot_id_2": {...}
        }

    Empty by default. Patch-window workflow: after D1, populate this file
    with per-opponent adjustments derived from hand-history analysis.
    Used in decision points that consult `_opp_profile_get(bot_id, key)`.
    """
    data_dir = os.environ.get("BOT_DATA_DIR")
    paths = []
    if data_dir:
        paths.append(os.path.join(data_dir, "opponent_profiles.json"))
    paths.append(os.path.join(os.path.dirname(__file__), "data", "opponent_profiles.json"))
    for path in paths:
        try:
            with open(path, "r", encoding="utf-8") as handle:
                data = json.load(handle)
...
_OPPONENT_PROFILES = _load_opponent_profiles()


def _opp_profile_adj(bot_id, key, default=0.0):
    """Look up a per-opponent override. Returns default if not present.

    Used at decision points where we want to encode specific exploits
    against known opponents. With an empty profile file, this returns
    the default in all cases — identical behavior to v5_partition.
    """
    if not bot_id:
        return default
    profile = _OPPONENT_PROFILES.get(bot_id) or {}
...
def _decide_postflop(state):
    ...
    if villain["actions"] >= 8 and villain["raise_rate"] > 0.55:
        ...
        needed += 0.015
    # Per-opponent override hook: empty file means 0.0 adjustment (identical
    # to v5_partition). Populate data/opponent_profiles.json during patch
    # window with bot_id -> {"needed_adj": ±X} for exploits.
    needed += _opp_profile_adj(villain.get("bot_id"), "needed_adj", 0.0)
    ...
    for entry in reversed(state.get("action_log") or []):
        if entry.get("action") in ("raise", "all_in"):
            ...
            return {
                ...
                "bot_id": bot_id,  # NEW: enables per-opponent profile lookups
            }
    fallback = _table_profile()
    fallback["bot_id"] = None
    return fallback
