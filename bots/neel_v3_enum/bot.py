...
def _estimate_equity(state, opponents):
    ...
    # River heads-up: enumerate exactly instead of sampling. 1225 opp combos
    # at ~1us each = ~1.2ms — well under budget — eliminates ~3% MC noise.
    if street == "river" and opponents == 1 and len(board) == 5:
        floor = _seat_range_floor(state, seats[0])
        return _enumerate_river_hu_equity(hero, board, deck, floor)
...
def _enumerate_river_hu_equity(hero, board, deck, floor):
    """Exact heads-up river equity by enumerating all 2-card opp combos.

    Each candidate combo is weighted by its preflop_score's compatibility
    with the opponent's range floor. Weight matches the stochastic rejection
    sampler used elsewhere (`loosen = rand * 14`) — a hand with score X is
    in-range with probability min(1, max(0, score + 14 - floor) / 14).
    Hero score is constant once board is fixed, so we compute it once.
    """
    ...
    for i in range(n):
        ...
        for j in range(i + 1, n):
            c2 = deck[j]
            ps = _preflop_score_cards(c1, c2)
            slack = ps + 14.0 - floor
            if slack <= 0.0:
                continue
            weight = 1.0 if slack >= 14.0 else slack / 14.0
