...
def is_maniac(bot_id: str, min_sample: int = 6) -> bool:
    """True if opponent has shown high VPIP+PFR pattern (random/aggressive bot).
    Triggers exploit: don't call their huge bets without monsters."""
    if bot_id not in OPPONENTS:
        return False
    opp = OPPONENTS[bot_id]
    if opp.get("vpip_chances", 0) < min_sample:
        return False
    vpip = opp["vpip_actions"] / max(opp["vpip_chances"], 1)
    pfr = opp["pfr_actions"] / max(opp["pfr_chances"], 1)
    # Maniac signature: VPIP > 50% AND PFR > 40% (lowered thresholds)
    return vpip > 0.50 and pfr > 0.40


def is_calling_station(bot_id: str, min_sample: int = 8) -> bool:
    """True if opponent calls way too much - never bluff them.
    Detected via: high VPIP + low PFR (calls a lot, raises rarely)."""
    if bot_id not in OPPONENTS:
        return False
    opp = OPPONENTS[bot_id]
    if opp.get("vpip_chances", 0) < min_sample:
    ...
    vpip = opp["vpip_actions"] / max(opp["vpip_chances"], 1)
    pfr = opp["pfr_actions"] / max(opp["pfr_chances"], 1)
    # Station signature: VPIP > 45%, PFR < 15% (calls everything, rarely raises)
    return vpip > 0.45 and pfr < 0.15


def any_active_maniac(state: dict) -> bool:
    """True if any active aggressor in this hand is flagged as a maniac."""
    log = state.get("action_log", [])
    me = state["seat_to_act"]
    for e in log:
        if e.get("action") in ("raise", "all_in") and e.get("seat") != me:
            ...
            if bot_id and is_maniac(bot_id):
                return True
    return False


def stack_risked_pct(state: dict, owed: int) -> float:
    """Fraction of starting stack at risk if we call this bet."""
    stack = state["your_stack"]
    bet_so_far = state["your_bet_this_street"]
    total_invested_if_call = bet_so_far + owed
    starting_stack_estimate = stack + bet_so_far  # current + already-bet
...
def decide_preflop_6max(state: dict, position: str, hand: str, cfg: Config) -> dict:
    ...
    if scenario == "open":
        ...
        # If there's a known maniac in the hand, tighten significantly
        # (they'll often 3-bet shove making our marginal opens unprofitable)
        has_maniac = False
        for p in state["players"]:
            if p.get("seat") != state["seat_to_act"] and not p.get("is_folded"):
                if is_maniac(p["bot_id"]):
                    has_maniac = True
                    break

        if has_maniac:
            # Tighten to top 12% from any position (66+/AJs+/KQs/AQo+)
            tight_set = expand_range("66+,AJs+,KQs,AQo+,AKo")
            rfi_set = rfi_set & tight_set
    ...
    if scenario == "face_3bet_as_raiser":
        # Stack-preservation: if a maniac is shoving, we need a true monster
        risk_pct = (state["amount_owed"] + state["your_bet_this_street"]) / (state["your_stack"] + state["your_bet_this_street"])
        facing_maniac = any_active_maniac(state)

        if facing_maniac and risk_pct >= 0.30:
            # Vs maniac shove: only call/raise with QQ+, AKs (top of range)
            if hand in {"AA", "KK", "QQ", "AKs"}:
                return {"action": "all_in"}
            return {"action": "fold"}

        # Compute what our standard 4-bet would cost
        ip = position in ("CO", "BTN")
        current = state["current_bet"]
        ...
        # If 4-betting commits >25% of our stack, jam-or-fold instead
        # (prevents bleed-by-raise-then-fold pattern)
        if fourbet_risk >= 0.25:
            if hand in FIVEBET_SET or hand in {"AA", "KK", "QQ", "AKs"}:
                return {"action": "all_in"}
            return {"action": "fold"}

        # If stack is shallow (<40bb) treat as jam-or-fold
        bb_in_stack = state["your_stack"] / BIG_BLIND
        if bb_in_stack < 40:
            if hand in FIVEBET_SET or hand in {"AA", "KK", "QQ", "AKs"}:
                return {"action": "all_in"}
            return {"action": "fold"}

        # 4-bet our value range (normal case)
        if hand in FOURBET_VALUE_SET:
            return {"action": "raise", "amount": safe_raise_amount(state, proposed_4bet)}

        # 4-bet bluff with blockers - DISABLED vs maniacs (they don't fold)
        if hand in FOURBET_BLUFF_SET and position in ("CO", "BTN") and not facing_maniac:
            if random.random() < 0.30:
                return {"action": "raise", "amount": safe_raise_amount(state, proposed_4bet)}

        # Value-call with strong-but-not-4-bet hands (only if cheap)
        if hand in {"JJ", "TT", "AKo", "AQs"} and owed <= state["your_stack"] * 0.15:
            return {"action": "call"}
    ...
    if scenario == "face_3bet_cold":
        risk_pct = (state["amount_owed"] + state["your_bet_this_street"]) / (state["your_stack"] + state["your_bet_this_street"])
        if any_active_maniac(state) and risk_pct >= 0.30:
            # Vs maniac in 3-bet pot: monsters only
            if hand in {"AA", "KK"}:
                return {"action": "all_in"}
            return {"action": "fold"}
    ...
    if scenario in ("face_4bet_as_raiser", "face_5bet_as_raiser"):
        risk_pct = (state["amount_owed"] + state["your_bet_this_street"]) / (state["your_stack"] + state["your_bet_this_street"])
        # Vs maniac in 4-bet+ pot, even tighter
        if any_active_maniac(state) and risk_pct >= 0.30:
            if hand == "AA":
                return {"action": "all_in"}
            return {"action": "fold"}
...
def decide_postflop(state: dict, position: str, cfg: Config) -> dict:
    ...
    # Detect opponent profile of the bot betting into us
    facing_maniac = any_active_maniac(state)
    facing_station = False
    if agg_seat is not None:
        agg_id = next((p["bot_id"] for p in state["players"] if p["seat"] == agg_seat), None)
        if agg_id and is_calling_station(agg_id):
            facing_station = True
    ...
    if can_check:
        ...
        if was_pf_aggressor and street == "flop":
            ...
            # Don't c-bet bluff into stations - they call with anything
        ...
        # Thin value IP - but ONLY if not vs station with weak hand
        if eq >= cfg.equity_thin_value and in_position:
            target = int(state["current_bet"] + pot * cfg.sizing_thin)
            return {"action": "raise", "amount": safe_raise_amount(state, target)}

        # Bluff frequency - DISABLED vs stations
        if facing_station:
            return {"action": "check"}
    ...
    # When facing a bet that risks a big chunk of our stack, require much
    # higher equity than pot odds suggest. Variance kills you in a 200-hand match.
    risk_pct = stack_risked_pct(state, owed)

    if risk_pct >= 0.30:
        # Calling 30%+ of stack: need monster hand or fold
        # Vs a maniac, even tighter (their shoves are wider, but we can't recover from a wrong call)
        threshold = 0.78 if facing_maniac else 0.72
        if eq >= threshold:
            return {"action": "call"}
        return {"action": "fold"}

    if risk_pct >= 0.15:
        # Calling 15-30% of stack: need solid equity buffer
        threshold = 0.65 if facing_maniac else 0.58
        if eq >= threshold:
    ...
    # === Normal-sized bet (under 15% of stack): standard logic ===
    # Big hand: raise for value
    ...
    # Decent equity vs pot odds (with safety buffer)
    if eq >= pot_odds + 0.08:
        return {"action": "call"}

    # Marginal: only call if very cheap
    if eq >= cfg.equity_call_threshold and owed <= pot * 0.20:
        return {"action": "call"}
