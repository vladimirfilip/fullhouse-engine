...
def _value_bet(state, pot, stack, equity, made, wet, opponents, spr):
    ...
    if opponents >= 3:
        frac += 0.08
    # PATCH (anti_maniac): cap value-bet aggression on low-SPR non-monsters.
    # Prevents committing stack with marginal hands on flop/turn when villain
    # can jam behind. Deploy if D1 shows commitment-line stack-offs.
    if spr <= 1.8 and made < 4 and equity < 0.72:
        frac = min(frac, 0.40)
